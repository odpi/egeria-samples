...
package org.odpi.openmetadata.samples.connectors.orgsync.rest;
...
public class RESTClient
{
     ...
     *
     * @param connectorName calling connector
     * @param url URL root of the application to call.
     * @param auditLog destination for log messages.
     *
     * @throws InvalidParameterException there is a problem creating the client-side components to issue any
     * REST API calls.
     */
    public RESTClient(String   connectorName,
                      String   url,
                      AuditLog auditLog) throws InvalidParameterException
    {
        final String  methodName = "RESTClient(no authentication)";

        this.serverName = connectorName;
        this.url = url;
        this.auditLog = auditLog;

        RESTClientFactory factory = new RESTClientFactory(connectorName, url);
        ...
        {
            throw new InvalidParameterException(OrgSyncIntegrationConnectorErrorCode.NO_URL.getMessageDefinition(connectorName, error.getMessage()),
                                                this.getClass().getName(),
                                                methodName,
                                                error,
                                                "url or connectorName");
        }
    }
                         ...
                         AuditLog auditLog) throws InvalidParameterException
    {
        ...
        {
            throw new InvalidParameterException(OrgSyncIntegrationConnectorErrorCode.NO_URL.getMessageDefinition(serverName, error.getMessage()),
                                                this.getClass().getName(),
                                                methodName,
                                                error,
                                                "url or serverName");
                                      ...
                                      Exception error) throws PropertyServerException
    {
        if (auditLog != null)
        {
            auditLog.logException(methodName,
                                  OrgSyncIntegrationConnectorAuditCode.CLIENT_SIDE_REST_API_ERROR.getMessageDefinition(methodName,
                                                                                                                       serverName,
                                                                                                                       url,
                                                                                                                       error.getMessage()),
                                  error);
        ...
        throw new PropertyServerException(OrgSyncIntegrationConnectorErrorCode.CLIENT_SIDE_REST_API_ERROR.getMessageDefinition(methodName,
                                                                                                                               serverName,
                                                                                                                               url,
                                                                                                                               error.getMessage()),
                                          this.getClass().getName(),
